...
LABEL_NAMES = np.asarray(['background', 'pointer', 'scale']) # 分割类别标签
FULL_LABEL_MAP = np.arange(len(LABEL_NAMES)).reshape(len(LABEL_NAMES), 1)
FULL_COLOR_MAP = label_to_color_image(FULL_LABEL_MAP)

'''
imgfile = './data/test/demo.bmp'
pngfile = './data/results/demo.png'
img = cv2.imread(imgfile, 1)
img = img[:,:,::-1]
seg_map = cv2.imread(pngfile, 0)
seg_map = cv2.resize(seg_map, (img.shape[1], img.shape[0]))
# seg_map = seg_map % 2
